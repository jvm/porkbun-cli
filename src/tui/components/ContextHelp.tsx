...
export function ContextHelp({ theme, context, onClose }: ContextHelpProps) {
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor={theme.colors.primary} padding={1}>
      <Box marginBottom={1}>
        <Text bold color={theme.colors.primary}>Help: {context}</Text>
      </Box>
      ...
      <Box marginTop={1} flexDirection="column">
        ...
        {helpContent.map((item, index) => (
          <Text key={index}>  {item.key.padEnd(15)} {item.description}</Text>
        ))}
      </Box>

      <Box marginTop={1} flexDirection="column">
        <Text bold color={theme.colors.warning}>Safety:</Text>
      ...
      <Box marginTop={1} flexDirection="column">
        <Text bold color={theme.colors.muted}>Web-Only Features:</Text>
