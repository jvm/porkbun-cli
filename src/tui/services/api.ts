...
export class TuiApiService {
  ...
  async getDomains(
    query: DomainQuery = {},
  ): Promise<ResourceState<{ domains: NormalizedDomain[]; count: number }>> {
    try {
      const data = await this.client.request(requireOperation("getDomains"), {
        query: {
          ...
          tlds: query.tlds,
          autoRenew:
            query.autoRenew === true ? "yes" : query.autoRenew === false ? "no" : undefined,
          apiAccess:
            query.apiAccess === true ? "yes" : query.apiAccess === false ? "no" : undefined,
  ...
  ): Promise<ResourceState<Record<string, unknown>>> {
    try {
  ...
  async getSslBundle(domain: string): Promise<ResourceState<NormalizedSslBundle>> {
    try {
      ...
      return {
        ...
        data: {
          certificateChain:
            typeof record.certificatechain === "string" ? record.certificatechain : undefined,
  ...
  async getPricing(): Promise<
    ResourceState<Map<string, { registration: string; renewal: string; transfer: string }>>
  > {
    try {
  ...
  async getTldPrice(
    domain: string,
    kind: "registration" | "renewal" | "transfer",
  ): Promise<string | undefined> {
  ...
  async getBalance(): Promise<ResourceState<NormalizedAccountBalance>> {
    try {
      ...
      const display =
        typeof record.display === "string" ? record.display : `$${(balance / 100).toFixed(2)}`;
      return {
        status: "loaded",
  ...
  async getApiSettings(): Promise<ResourceState<NormalizedApiSettings>> {
    try {
      ...
      const record = asRecord(data);
      const monthlySpend =
        typeof record.monthlySpend === "number" ? record.monthlySpend : undefined;
      return {
        status: "loaded",
        data: {
...
function normalizeGlueResponse(
    ...
    return records
      .map((r) => {
        ...
        return {
          ...
          maxSigLife:
            raw.maxSigLife !== undefined ? parseInt(String(raw.maxSigLife), 10) : undefined,
          keyDataFlags:
            raw.keyDataFlags !== undefined ? parseInt(String(raw.keyDataFlags), 10) : undefined,
          keyDataProtocol:
            raw.keyDataProtocol !== undefined
              ? parseInt(String(raw.keyDataProtocol), 10)
              : undefined,
          keyDataAlgo:
            raw.keyDataAlgo !== undefined ? parseInt(String(raw.keyDataAlgo), 10) : undefined,
...
function asArray(value: unknown): Array<Record<string, unknown>> {
  if (Array.isArray(value))
    return value.filter((v) => typeof v === "object" && v !== null) as Array<
