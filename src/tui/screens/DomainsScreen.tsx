...
import {
  ...
  EmptyState,
  StaleBanner,
} from "../components/StatusComponents.js";
...
export function DomainsScreen({
  ...
  const [autoRenewMode, setAutoRenewMode] = useState<
    "idle" | "confirm" | "submitting" | "success" | "error"
  >("idle");
    ...
    if (char === "a" || char === "A") {
      const targets =
        selectedDomains.size > 0
          ...
          : filteredDomains.at(selectedIndex)
            ? [filteredDomains.at(selectedIndex)!.domain]
            : [];
    ...
    <Box flexDirection="column" flexGrow={1}>
      ...
      {filteredDomains.length === 0 ? (
        <EmptyState
          ...
          details={debouncedSearch ? `No matches for "${debouncedSearch}"` : undefined}
          theme={theme}
        />
      ) : (
        <VirtualList
        ...
        <MutationConfirm
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              setAutoRenewSuccess(
                `Auto-renew ${autoRenewNewState ? "enabled" : "disabled"} for ${autoRenewTargets.length} domain(s)`,
              );
      ...
      {(autoRenewMode === "success" || autoRenewMode === "error") && (
        <Box marginTop={1} flexDirection="column">
          {autoRenewSuccess && <Text color={theme.colors.success}>✓ {autoRenewSuccess}</Text>}
          {autoRenewError && <Text color={theme.colors.danger}>✗ {autoRenewError}</Text>}
      ...
      {/* Context Help Modal */}
      {showContextHelp && (
        <ContextHelp theme={theme} context="domains" onClose={() => setShowContextHelp(false)} />
      )}
    </Box>
  );
}
...
function DomainRow({ domain, isSelected, isMultiSelected, theme }: DomainRowProps) {
  ...
  return (
    <Box>
      ...
      <Text
        color={domain.autoRenew ? theme.colors.success : theme.colors.danger}
      ...
      <Text
        color={domain.apiAccess ? theme.colors.success : theme.colors.warning}
        backgroundColor={selBg}
      >
        {" "}
      ...
      {attention && (
        <Text color={attention.color} backgroundColor={selBg}>
          {" "}
          {attention.icon}
        </Text>
      )}
...
function getAttentionIndicator(
  domain: NormalizedDomain,
  theme: Theme,
): { icon: string; color: string | undefined } | null {
