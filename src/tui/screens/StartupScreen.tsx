...
export function StartupScreen({ service, theme, onSuccess, credentialSource }: StartupScreenProps) {
  ...
  useEffect(() => {
    if (!credentialSource) {
      listProfiles().then(p => {
        ...
        if (p.length === 0) {
          setPhase('error');
          setError(new Error('No credentials found. Run porkbun auth login to save credentials.'));
        } else if (p.length === 1) {
          setPhase('validating');
        }
      }).catch(err => {
  ...
  useEffect(() => {
    ...
    service.ping().then(result => {
      if (result.status === 'loaded' && result.data?.credentialsValid) {
        ...
        setPhase('error');
        setError(new Error(result.error?.message ?? 'Authentication failed. Credentials are not valid.'));
      }
    });
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor={theme.colors.primary} padding={1}>
      <Box marginBottom={1}>
        <Text bold color={theme.colors.primary}>Select a profile:</Text>
      </Box>
      {profiles.map((profile, i) => (
        <Box key={profile}>
          ...
          >
            {i === selectedProfile ? theme.icons.selected : theme.icons.unselected}
            {' '}{profile}
          </Text>
        </Box>
      ))}
