...
function Header({ theme, profileName, credentialSource, balanceCents }: {
  theme: Theme;
  profileName?: string;
  credentialSource?: 'flags' | 'env' | 'profile';
  balanceCents?: number;
  ...
  return (
    <Box justifyContent="space-between" width="100%">
      <Box>
        <Text bold color={theme.colors.primary}>Porkbun TUI</Text>
...
function AccountScreen({ theme, balanceCents, onBack }: {
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Text bold color={theme.colors.primary}>Account</Text>
      <Box marginTop={1}>
        <Text dimColor>Balance: </Text>
        <Text>{balanceCents !== undefined ? `$${(balanceCents / 100).toFixed(2)} (${balanceCents} cents)` : '(loading...)'}</Text>
      </Box>
    </Box>
  );
}
...
function HelpScreen({ theme, onBack }: { theme: Theme; onBack: () => void }) {
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Text bold color={theme.colors.primary}>Help</Text>
