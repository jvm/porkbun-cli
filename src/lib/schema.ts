...
export function buildCliSchema(commandPath?: string): Record<string, unknown> {
  ...
  return {
    ...
    commands: commandPath ? filterCommands(commands, commandPath.split(".").filter(Boolean)) : commands,
    errors: [
      ...
      { kind: "validation", retryable: false, description: "The API rejected a malformed or unavailable resource." },
      { kind: "not_found", retryable: false, description: "The requested resource does not exist." },
      { kind: "conflict", retryable: false, description: "The request conflicts with existing state or idempotency constraints." },
      ...
      { kind: "api_error", retryable: false, description: "An API error not covered by a more specific kind." }
    ],
...
function addStaticCommands(root: SchemaCommand[]): void {
  ...
  addStatic(root, ["auth", "logout"], "Delete a saved profile.", true, [{ name: "--profile", type: "string", required: false }]);
  addStatic(root, ["auth", "profiles"], "List saved profiles.", false, []);
  addStatic(root, ["auth", "whoami"], "Show credential source and validate credentials with ping.", false, []);
  ...
  addStatic(root, ["schema"], "Print the CLI schema.", false, [{ name: "command", type: "string", required: false }]);
}
