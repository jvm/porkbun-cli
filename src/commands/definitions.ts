...
const dnsEditByNameTypeOptions: CommandOptionDefinition[] = dnsWriteOptions.filter((option) => option.name !== "type" && option.name !== "name");
...
export const CLI_COMMANDS: CliCommandDefinition[] = [
  ...
  },
  {
    ...
    listKey: "domains",
    options: [
      queryOption("start", "--start <offset>", "API pagination offset.", "integer", "start", "integer"),
      queryOption("includeLabels", "--include-labels <yes|no>", "Include Porkbun label metadata.", "string", "includeLabels"),
      queryOption("domain", "--domain <domain>", "Exact domain match.", "string", "domain"),
      queryOption("nameContains", "--name-contains <text>", "Substring match against domain names.", "string", "nameContains"),
      queryOption("expiringWithinDays", "--expiring-within-days <days>", "Filter to domains expiring within N days.", "integer", "expiringWithinDays", "integer"),
      arrayOption("tld", "--tld <tld>", "Filter to a TLD. Repeat or use comma-separated values.", "tlds"),
      queryOption("autoRenew", "--auto-renew <yes|no>", "Filter by auto-renew status.", "string", "autoRenew"),
      queryOption("apiAccess", "--api-access <yes|no>", "Filter by API access status.", "string", "apiAccess"),
      queryOption("sortName", "--sort-name <field>", "Sort field.", "string", "sortName"),
      queryOption("sortDirection", "--sort-direction <asc|desc>", "Sort direction.", "string", "sortDirection")
    ],
  ...
  },
  {
    ...
    options: [bodyArrayOption("ip", "--ip <address>", "IP address for the glue record. Repeat for multiple addresses.", "ips", true)]
  },
  {
  ...
  },
  {
    ...
    args: [domainArg],
    options: [
      bodyOption("subdomain", "--subdomain <name>", "Forward subdomain. Omit for root.", "string", "subdomain"),
      bodyOption("location", "--location <url>", "Target URL.", "string", "location", undefined, true),
      bodyOption("type", "--type <permanent|temporary>", "Redirect type.", "string", "type", undefined, true),
      bodyOption("includePath", "--include-path <yes|no>", "Whether to include the incoming path.", "string", "includePath", undefined, true),
      bodyOption("wildcard", "--wildcard <yes|no>", "Whether to include wildcard subdomains.", "string", "wildcard", undefined, true)
    ]
  },
  {
  ...
  },
  {
    ...
    args: [
      domainArg,
      { name: "type", syntax: "<type>", description: "DNS record type.", type: "string", target: "path" },
  ...
  },
  {
    ...
    args: [domainArg],
    options: [
      bodyOption("keyTag", "--key-tag <keytag>", "DNSSEC key tag.", "integer", "keyTag", "integer", true),
      bodyOption("alg", "--alg <algorithm>", "DNSSEC algorithm.", "integer", "alg", "integer", true),
      bodyOption("digestType", "--digest-type <type>", "DNSSEC digest type.", "integer", "digestType", "integer", true),
      bodyOption("digest", "--digest <digest>", "DNSSEC digest.", "string", "digest", undefined, true),
      bodyOption("keyDataFlags", "--key-data-flags <flags>", "Optional key data flags.", "integer", "keyDataFlags", "integer"),
      bodyOption("keyDataProtocol", "--key-data-protocol <protocol>", "Optional key data protocol.", "integer", "keyDataProtocol", "integer"),
      bodyOption("keyDataAlgo", "--key-data-algo <algorithm>", "Optional key data algorithm.", "integer", "keyDataAlgo", "integer"),
      bodyOption("keyDataPubKey", "--key-data-pub-key <key>", "Optional key data public key.", "string", "keyDataPubKey"),
      bodyOption("maxSigLife", "--max-sig-life <seconds>", "Optional max signature life.", "integer", "maxSigLife", "integer")
    ]
  },
  {
    ...
    args: [
      domainArg,
      { name: "keytag", syntax: "<keytag>", description: "DNSSEC key tag.", type: "string", target: "path" }
    ]
  },
  {
    path: ["ssl", "retrieve"],
  ...
  },
  {
    ...
    options: [
      bodyOption("emailAddress", "--email-address <email>", "Full email address.", "string", "emailAddress", undefined, true),
      bodyOption("password", "--password <password>", "New password. Prefer --password-stdin in scripts.", "string", "password"),
      {
        name: "passwordStdin",
  ...
  },
  {
    ...
    listKey: "domains",
    options: [
      queryOption("query", "--query <text>", "Search query. Prefix terms with - to exclude.", "string", "query"),
      arrayOption("tld", "--tld <tld>", "Filter to a TLD. Repeat or use comma-separated values.", "tlds"),
      queryOption("sldLengthMin", "--sld-length-min <n>", "Minimum SLD length.", "integer", "sldLengthMin", "integer"),
      queryOption("sldLengthMax", "--sld-length-max <n>", "Maximum SLD length.", "integer", "sldLengthMax", "integer"),
      queryOption("sortName", "--sort-name <field>", "Sort field: domain, tld, price, or sld_length.", "string", "sortName"),
      queryOption("sortDirection", "--sort-direction <asc|desc>", "Sort direction.", "string", "sortDirection"),
      queryOption("start", "--start <offset>", "API pagination offset.", "integer", "start", "integer")
    ],
    build: ({ options, globalOptions }) => ({
...
export function defaultInvocation(definition: CliCommandDefinition, context: BuildContext): OperationInvocation {
  return {
...
export function targetOptions(options: Record<string, unknown>, target: Target): Record<string, unknown> {
...
function normalizeValue(value: unknown): unknown {
  if (Array.isArray(value)) {
    ...
    const cleaned = flat.map((entry) => (typeof entry === "string" ? entry.trim() : entry)).filter((entry) => entry !== "");
...
function stringArray(value: unknown): string[] {
  ...
  if (typeof normalized === "string") return normalized.split(",").map((entry) => entry.trim()).filter(Boolean);
  return [];
}
